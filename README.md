...
- **Tier 0 is the wired deliverable; tiers 1–4 are walkable design artifacts.** Tier 0 is the literal Kargo Quickstart and is the tier I exercised against my Akuity trial-account Argo CD + Kargo instances. Tiers 1–4 are not stood up against live clusters — they are coherent, applyable manifests that extend the same patterns, but I did not deploy them end-to-end. Calling this out so the technical review doesn't trip over a "worked on my machine" claim I can't back up. The promotion shape, AppProject scoping, sync windows, and Kargo step pipelines in tiers 1–4 are byte-consistent with what runs in tier 0, so the design substrate is the part to evaluate; the apply-and-watch-it-converge part is only honest for tier 0.
- **Akuity-managed Argo CD, not self-hosted.** My trial-account Akuity Argo CD instance is the deployment target for tier 0 — I did not stand up self-hosted Argo CD anywhere in this repo. The agent connection model is what tier 0 is selling, and verifying tier 0 manifests against an actual Akuity-managed instance is the cleanest way to demonstrate the lift it removes. Self-hosting becomes viable in raw cost terms at tier 3, but rarely in opportunity-cost terms.
